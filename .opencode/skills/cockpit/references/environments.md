...
for you or prefer the host platform:
...
cached id, the host platform, or an implicit platform default. If one row is
compatible, report which id was selected; do not silently substitute a physical
device for a simulator (or the reverse).
...
`deviceAmbiguous` means the command found several live choices and requires one
exact id. `deviceNotFound` means the requested device is offline, unauthorized,
or not booted; repair that same device and rediscover it. Cockpit never switches
to another device or starts a second app as recovery.
...
operations work. Verify those capabilities with a read-only inspect, a
system-first screenshot, and wait before mutating the app. Keep the same device
id through the whole session and rediscover only that device after a disconnect.
