...
void main() {
  ...
  test('skill teaches the short Flutter development workflow', () {
    ...
    for (final contract in <String>[
      ...
      'Development handles are short lowercase base-36 values',
      '--session HANDLE',
      ...
      'cockpit dev tree',
      'cockpit dev open',
      'source is the default locator channel during\ndevelopment',
      'execute the action without a pre-inspect round trip',
      'runtime facts source cannot determine',
      ...
      'default bounded `mounted` targets',
      'cockpit dev dismiss',
      'cockpit dev back',
      'resolveBlockers',
      ...
      'merged ancestor `Semantics` never makes\npassive descendants actionable',
      '`IgnorePointer(ignoring:\ntrue)` or `AbsorbPointer(absorbing: true)`',
      'multiple delegated controls leave state unresolved instead\nof guessing',
      "Execute only the `sel` whose own `can` advertises the command",
