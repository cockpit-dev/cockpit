...
.archive-list { display: grid; gap: 7px; margin-top: 12px; }
.archive-path { display: flex; align-items: center; gap: 8px; min-width: 0; padding: 8px 10px; border-radius: 9px; background: var(--surface-2); }
.archive-path code { min-width: 0; flex: 1; overflow-wrap: anywhere; color: var(--text-soft); }
.archive-path .quiet-button { flex: none; min-height: 30px; padding: 5px 9px; }
...
<div class="shell">
  ...
  <main class="workspace">
    ...
    <section class="panel hidden" id="archive-panel"><div class="panel-head"><div><h2>Large capture archive</h2><p>Long-running captures stay outside this HTML payload. Open the manifest or a JSONL chunk when you need the complete event stream.</p></div><span class="subtle" id="archive-summary">—</span></div><div class="detail-grid" id="archive-grid"></div><div class="archive-list" id="archive-list"></div></section>
...
(function () {
  ...
  var renderCoverage = function () {
    ...
    var rows = [
      ...
      ['JSONL archive', r.stream ? 'available' : 'unavailable', r.stream ? nf.format(number(r.stream.events)) + ' events in ' + nf.format((r.stream.chunks || []).length) + ' chunks' : 'Pass archive: to profile for long captures'],
  ...
  var renderComparison = function () {
    ...
    body.innerHTML = reports.map(function (item, index) {
      ...
      return '<tr class="' + (index === state.report ? 'current' : '') + '"><td><button class="quiet-button" type="button" data-report-index="' + index + '">' + esc(label) + '</button></td><td><code>' + esc(r.platform || 'unknown') + '</code></td><td>' + esc(r.build || 'unknown') + '</td><td>' + nf.format(count) + '</td><td class="' + (jank ? 'bad' : '') + '">' + nf.format(jank) + '</td><td>' + esc(fps) + '</td><td>' + esc(total.p90 == null ? '—' : us(total.p90)) + '</td><td>' + esc(total.max == null ? '—' : us(total.max)) + '</td><td>' + esc(duration(r.durationMs)) + '</td><td>' + esc(m ? bytes(m.peak) : '—') + '</td><td>' + esc(m ? bytes(m.delta) : '—') + '</td></tr>';
    }).join('') || '<tr><td colspan="11"><div class="empty">No captures.</div></td></tr>';
  ...
  var renderArchive = function () {
    var archive = report().stream || null;
    var panel = el('archive-panel');
    if (!archive) { panel.classList.add('hidden'); return; }
    panel.classList.remove('hidden');
    el('archive-summary').textContent = String(archive.state || 'active') + ' · ' + nf.format(number(archive.records)) + ' records';
    var cells = [
      ['Format', archive.fmt || 'jsonl'],
      ['Mode', archive.mode || 'lossless'],
      ['Events', nf.format(number(archive.events))],
      ['Frames', nf.format(number(archive.frames))],
      ['Bytes', bytes(archive.bytes)],
      ['Dropped', nf.format(number(archive.drop))],
      ['Manifest', archive.manifest || '—'],
    ];
    el('archive-grid').innerHTML = cells.map(function (cell) { return '<div class="detail-cell"><span>' + esc(cell[0]) + '</span><strong class="breakable">' + esc(cell[1]) + '</strong></div>'; }).join('');
    var chunks = Array.isArray(archive.chunks) ? archive.chunks : [];
    el('archive-list').innerHTML = chunks.length ? '<div class="subtle">Chunks</div>' + chunks.map(function (path) { return '<div class="archive-path"><code>' + esc(path) + '</code><button class="quiet-button" type="button" data-copy-path="' + esc(path) + '">Copy path</button></div>'; }).join('') : '<div class="empty">No completed chunk has been indexed yet.</div>';
    el('archive-list').querySelectorAll('[data-copy-path]').forEach(function (button) { button.addEventListener('click', function () { var path = button.getAttribute('data-copy-path') || ''; if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(path); button.textContent = 'Copied'; setTimeout(function () { button.textContent = 'Copy path'; }, 1000); }); });
  };
  ...
  var renderSelector = function () { var select = el('report-select'); select.innerHTML = reports.map(function (item, index) { var r = item.report || {}; var label = item.label || ('Capture ' + (index + 1)); var suffix = [r.platform, r.build].filter(Boolean).join(' · '); return '<option value="' + index + '">' + esc(suffix ? label + ' · ' + suffix : label) + '</option>'; }).join(''); select.value = state.report; };
  var render = function () { hideTooltip(); renderSelector(); renderMeta(); renderHealth(); renderMetrics(); renderStartup(); renderCoverage(); renderPlugins(); renderDevTools(); renderEventSummary(); renderStalls(); renderCategories(); renderResources(); renderComparison(); renderDetails(); renderArchive(); renderFrames(); renderEvents(); renderCodeEvidence(); renderHotspots(); drawStartupChart(); drawFrameChart(); drawEventChart(); drawPhaseChart(); drawResourceChart(); drawCacheChart(); drawJankChart(); drawCadenceChart(); drawCacheTrendChart(); drawPipelineChart(); drawGcChart(); drawCategoryCostChart(); drawHotspotChart(); drawFlameChart(); drawCpuChart(); drawHeapChart(); drawHeapTrendChart(); drawVmMemoryChart(); drawRebuildChart(); };
