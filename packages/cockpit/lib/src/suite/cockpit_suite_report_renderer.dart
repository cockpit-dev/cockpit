...
<main class="report-main">
  ...
  <section class="lens" id="coverage" role="tabpanel" aria-labelledby="tab-coverage">${_product(bundle)}</section>
  <section class="lens" id="executions" role="tabpanel" aria-labelledby="tab-executions">${_quality(bundle)}</section>
  <section class="lens" id="evidence" role="tabpanel" aria-labelledby="tab-evidence">${_evidence(bundle)}</section>
  <section class="lens" id="diagnostics" role="tabpanel" aria-labelledby="tab-diagnostics">${_engineering(bundle)}</section>
...
String _overview(CockpitTestReportBundle bundle, {required bool gatePassed}) {
  ...
  return '''$alert<section class="section"><div class="summary-band"><div class="summary-verdict"><span class="subtle">Release decision</span><strong>${gatePassed ? 'Passed' : 'Needs review'}</strong><p>${gatePassed ? 'No failed, blocked, incomplete, or flaky cases.' : 'Open diagnostics before accepting this run.'}</p></div><div class="count-grid"><div class="count-item"><span>Passed</span><strong>${counts.passed}</strong></div><div class="count-item"><span>Failed</span><strong>${counts.failed}</strong></div><div class="count-item"><span>Blocked</span><strong>${counts.blocked}</strong></div><div class="count-item"><span>Flaky</span><strong>${counts.flaky}</strong></div><div class="count-item"><span>Skipped</span><strong>${counts.skipped}</strong></div><div class="count-item"><span>Cancelled</span><strong>${counts.cancelled}</strong></div><div class="count-item"><span>Interrupted</span><strong>${counts.interrupted}</strong></div><div class="count-item"><span>Internal error</span><strong>${counts.internalError}</strong></div></div></div></section>
...
String _actionTable(CockpitTestSuiteReport report) {
  ...
  for (final testCase in actionable) {
    final caseAnchor = _caseAnchor(testCase);
    rows.write(
      '''<tr><td><strong><a href="#$caseAnchor">${_html(testCase.entryId)}</a></strong><span class="identity">${_html(testCase.caseId)}</span></td><td>${_status(testCase.outcome.name)}${testCase.stability == CockpitRunStability.flaky ? '<span class="stability">flaky</span>' : ''}</td><td>${_html(_failureMessage(testCase))}</td><td>${testCase.attempts.length}</td></tr>''',
    );
  }
...
String _quality(CockpitTestReportBundle bundle) {
  ...
  for (final execution in bundle.executions) {
    ...
    content.write(
      '''<details class="execution" id="$executionAnchor" data-filter-item data-status="${_attribute(result.outcome.name)}" data-search="${_attribute(search)}"><summary>${_status(result.outcome.name)} <span>${_html(title)}</span><span class="subtle">${_html(_humanize(execution.role.name))} · $passed/${result.steps.length} steps · ${_html(_duration(result.durationMs))}</span></summary><div class="execution-body"><div class="case-meta"><span>Attempt <code>${_html(result.context.attemptId)}</code></span><span>Target <code>${_html(result.targetId)}</code></span><span>${_html(result.platform)}</span><span>${_html(result.requestedPlane.name)} → ${_html(result.actualPlane?.name ?? 'unresolved')}</span><span>${_html(_humanize(result.context.authorizationMode.name))} authorization</span></div>${_primaryError(result)}${_steps(execution, engineering: false)}${_cleanupErrors(result)}</div></details>''',
    );
  }
  if (content.isEmpty) {
    content.write(
...
String _engineering(CockpitTestReportBundle bundle) {
  ...
  for (final execution in prioritized) {
    ...
    content.write(
      '''<details class="execution" id="$diagnosticAnchor" data-filter-item data-status="${_attribute(result.outcome.name)}" data-search="${_attribute(search)}" ${primary == null && result.cleanupErrors.isEmpty ? '' : 'open'}><summary>${_status(result.outcome.name)} <span>${_html(title)}</span><span class="subtle">${_html(result.requestedPlane.name)} → ${_html(result.actualPlane?.name ?? 'unresolved')} · ${_html(_duration(result.durationMs))}</span></summary><div class="execution-body">${_primaryError(result)}<div class="case-meta"><span>Attempt <code>${_html(result.context.attemptId)}</code></span><span>Target <code>${_html(result.targetId)}</code></span><span>${_html(result.platform)}</span><span>Engine ${_html(result.context.engineVersion)}</span><span>${_html(_humanize(result.context.authorizationMode.name))} authorization</span></div>${_steps(execution, engineering: true)}${_cleanupErrors(result)}${_artifactLinks(execution.artifacts)}</div></details>''',
    );
  }
